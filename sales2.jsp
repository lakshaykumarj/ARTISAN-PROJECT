...
<%
  ...
  try (Connection conn = DriverManager.getConnection(url, username, password)) {
    //--------------------------------------------------------------------------------------------------------
    // Retrieving TABLENAME,artisanName,productName,costPrice,quantity
    String username = request.getParameter("");
    int quantity = request.getParameter("");
    String productName=request.getParameter("");
    String artisan_Name= "",tablename="";
    double costPrice=0.0;
    String tablenameQuery = "select artisan_type_name,cost_price from artisan_type join product on artisan_type.artisan_type_id = product.artisan_type_id where product_name=? and quantity=?";
    String tablenameQuery2= "select product.tablename from tablenames join product on tablenames.tabname = product.tablename where product_name=?";
            ...
            try (PreparedStatement insertProductStmt = conn.prepareStatement(insertProductQuery)) {
              ...
              try (PreparedStatement balanceStmt = conn.prepareStatement(balanceQuery)) {
                ...
                try (ResultSet balanceRs = balanceStmt.executeQuery()) {
                  ...
                  String insertTransactionQuery = "INSERT INTO transaction (transaction_id, transaction_date, transaction_type, amount, new_balance, tablename) VALUES (transaction_seq.NEXTVAL, ?, 'Purchase', ?, ?, ?)";
              ...
              try (ResultSet findProductRs = findProductStmt.executeQuery()) {
                if (findProductRs.next()) {
                  ...
                  try (PreparedStatement updateProductStmt = conn.prepareStatement(updateProductQuery)) {
                    ...
                    // Calculate the amount
                    double amount = -(costPrice * quantity);
